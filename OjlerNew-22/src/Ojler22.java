...
public class Ojler22 {

	public static void main(String[] args) {
		Ojler22 o22= new Ojler22();
		//o22.sort();
		String ret[]=o22.sort1();
		/*Arrays.sort(o22.names);
		for(int i=0; i<o22.names.length; i++)
			if(! o22.names[i].equals(ret[i]) ){
				System.out.println( (i+1) + ". " + o22.names[i] + "\t" + ret[i] );
			}*/
			
		for(int i=0; i<o22.names.length; i++)
				System.out.println( (i+1) + ". " + ret[i] );
		//System.out.println( o22.maxNameLength());
		
	long sum=0L;
		for(int i=0; i<o22.names.length; i++)	{
			int s=0;
			for(int j=0; j<o22.names[i].length(); j++)
				s+=o22.names[i].charAt(j)-'A' +1;
			sum+=(long)s*(i+1);
		}
	...
	void sort(){
		for(int i=1;i<this.names.length; i++)
				for(int j=this.names.length-1; j>=i; j--)
					if( this.names[j-1].charAt(0)  > this.names[j].charAt(0) ){
							String temp= this.names[j-1];
							this.names[j-1]= this.names[j];
							this.names[j]=temp;
						}
		
		for(int k=1;k<maxNameLength(); k++)
			for(int i=1;i<this.names.length; i++)
				for(int j=this.names.length-1; j>=i; j--)
					if( k<this.names[j-1].length() &&  k< this.names[j].length()  )
						if( this.names[j-1].charAt(k-1) == this.names[j].charAt(k-1)   ){
							if( this.names[j-1].charAt(k)  > this.names[j].charAt(k) ){
								String temp= this.names[j-1];
								this.names[j-1]= this.names[j];
								this.names[j]=temp;
							}
							else if( this.names[j-1].charAt(k)  == this.names[j].charAt(k) && 
									   k==this.names[j].length()-1  &&  this.names[j-1].length()>k  ){
								String temp= this.names[j-1];
								this.names[j-1]= this.names[j];
								this.names[j]=temp;
							}
